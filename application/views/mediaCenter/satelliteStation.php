<div id="topClock" class="text-center"><span id="topClockTime"></span> | <?php echo $title;?></div>
<div id="container">
	<img class="play" src="<?php echo base_url()."images/play_icon.png";?>">
	<a href="<?php echo base_url()."index.php/Media/satellite";?>"><img class="close" src="<?php echo base_url()."images/close_icon.png";?>"></a>
	<h2 id="addressbook" class="text-center userhead">satellite radio</h2>
	<p class="text-center totalContact white">@<?php echo strtolower($name[0].str_replace("'", "", $lname));?></p>
	<div class="spotifySong">
	<div class="small-4 columns mediaBtns">
		<img src="<?php echo base_url()."images/pop2k_bw.png";?>"><p class="white text-center">(click to change)</p>
	</div>
	<div class="small-8 columns songOptions">
		<p>Current Station: <span class="white marginLeft1">Pop2k</span></p>
		<p>Current Song: <span class="white marginLeft">What's Love by Ashanti and Ja Rule</span></p>
	</div>
	<hr>
</div>
    <div class="tripList">
		<div class="small-9 push-1 columns recent">
			<ul class="small-block-grid-3 columns tripHead">
				<li>Time</li>
				<li>Artist</li>
				<li>Song</li>
			</ul>
			<ul class="small-block-grid-3 columns trips">
				<li>11:55</li>
				<li>Marron 5</li>
				<li>This Love</li>

			</ul>
			<ul class="small-block-grid-3 columns trips">
				<li>11:50</li>
				<li>Finger Eleven</li>
				<li>One Thing</li>
			</ul>
			<ul class="small-block-grid-3 columns trips">
				<li>11:45</li>
				<li>Usher</li>
				<li>Confessions Pt.2</li>
			</ul>
			<ul class="small-block-grid-3 columns trips">
				<li>11:40</li>
				<li>Ashlee Simpson</li>
				<li>Pieces of Me</li>
			</ul>
			<ul class="small-block-grid-3 columns trips">
				<li>11:35</li>
				<li>Switchfoot</li>
				<li>Meant to Live</li>
			</ul>
			<ul class="small-block-grid-3 columns trips">
				<li>11:30</li>
				<li>Hoobastank</li>
				<li>The Reason</li>
			</ul>
			<ul class="small-block-grid-3 columns trips">
				<li>11:25</li>
				<li>Kanye West</li>
				<li>Jesus Walks</li>
			</ul>

		</div>
		
</div>